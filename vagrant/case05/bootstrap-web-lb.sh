...
echo -e "==> 12. Create consul-template configuration folder and copy nginx.conf template"
sudo mkdir -p /etc/consul-templates
sudo cp /vagrant/config/consul-template-files/nginx-web.conf /etc/consul-templates/nginx.conf



if [ ! -f /var/log/13_install_nginx ]
then
    echo -e "==> 13. Install nginx"

    sudo apt-get install nginx -y

    touch /var/log/13_install_nginx
else
    echo -e "==> 13. Nginx is already installed"
fi


if [ ! -f /var/log/14_install_consul_template ]
then
    echo -e "==> 14. Download consul-template and copy to /usr/local/bin"
    ...
    touch /var/log/14_install_consul_template
else
    echo -e "==> 14. consul-template is already installed"
fi

# Copy an upstart script to /etc/init so the Consul agent will be restarted if we restart the virtual machine.
echo -e "==> 15. Copy an upstart script to /etc/init, so the Consul template and nginx will be restarted if we restart the virtual machine"
sudo cp /vagrant/config/consul-template-files/consul-template-web-upstart.conf /etc/init/consul-template.conf
sudo cp /vagrant/config/consul-template-files/nginx-upstart.conf /etc/init/nginx.conf

echo -e "==> 16. Start consul-template and nginx will be started via consul-template"
